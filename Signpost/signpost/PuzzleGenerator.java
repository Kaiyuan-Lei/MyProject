    ...
    private int makeForwardConnections(Model model) {
        ...
        for (Sq sq : model) {
            ...
            if (sq.successor() == null && sq.direction() != 0) {
                ArrayList<Sq> canConnectTo = new ArrayList<>();
                for (int i = 0; i < w; i++) {
                    for (int j = 0; j < h; j++) {
                        if (sq.connectable(model.get(i, j))) {
                            nFound++;
                            canConnectTo.add(model.get(i, j));
                            found = model.get(model.get(i, j));
                        }
                    }
                }
                if (sq.sequenceNum() != 0) {
                    for (Sq s : canConnectTo) {
                        if (s.sequenceNum() != 0) {
                            nFound = 1;
                            found = s;
                            break;
    ...
    private int makeBackwardConnections(Model model) {
        ...
        for (Sq sq : model) {
            ...
            if (sq.predecessor() == null && sq.sequenceNum() != 1) {
                ...
                for (int i = 0; i < w; i++) {
                    for (int j = 0; j < h; j++) {
                        if (model.get(i, j).connectable(sq)) {
                            nFound++;
                            canConnectTo.add(model.get(i, j));
                            found = model.get(i, j);
                        }
